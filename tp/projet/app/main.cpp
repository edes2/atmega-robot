...
// Contient tout les etats dans lesquels le robot peut etre.
enum Etats
{
    init,
    debutParcours,
...
int main()
{
    ...
    ChauffeurDuRobot marguerite;
    LED led(&DDRA, &PORTA, DDA6, DDA7);
    can can;
    CapteurIR capteurIR(&can, PA0);
    SuiveurMur suiveurMur(&capteurIR, &marguerite);
    InterpreteurLumiere photo1(&can, PA2), photo2(&can, PA3);

    int calibration = (photo1.lectureIntensite() + photo2.lectureIntensite()) / 2;
    SuiveurLumiere capteursPhoto(&marguerite, calibration+20, ECART_MILIEU+5, ECART_MILIEU-15, ECART_MILIEU+20, ECART_MILIEU-25);
    ...
    char buffer[50]; // Buffer pour le debug
    ...
    while (true)
    {
        ...
        DEBUG_PRINT("Mur distance: ");
        itoa(capteurIR.mesurerDistance(), buffer, 10);
        DEBUG_PRINT(buffer);
        DEBUG_PRINT("\n");
        ...
        {
            DEBUG_PRINT("EXEC enregistrement\n");
        ...
        { 
            ...
            _delay_ms(1000);
            led.Ambre();
            marguerite.avancer(0.6);
            _delay_ms(1500);
            marguerite.pivoterADroite(0.53);
            _delay_ms(600);
            marguerite.avancer(0.6);
            _delay_ms(3000);
            marguerite.pivoterADroite(0.53);
